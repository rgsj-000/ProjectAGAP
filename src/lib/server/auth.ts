...
export async function requireLguUser(roles?: LguRole[]) {
  ...
  const { data: profile } = await supabase.from("user_profiles").select("role,active_status").eq("id", user.id).single();
  if (!profile?.active_status) throw new AppError("FORBIDDEN", "This LGU account is inactive.", 403);
