    ...
    if (prepareSubView === "priority-barangays") {
      return (
        <span className="text-xs font-semibold text-slate-700">
          {language === "en" ? "Barangay Assessments" : "Mga Pagtatasa ng Barangay"}
        </span>
      );
    }
  ...
  return (
    <header className="sticky top-0 z-20 w-full border-b border-slate-200 bg-white">
      {/* Mobile utility header: branding is retained because the desktop sidebar is hidden.
          Connectivity is shown on a quiet second row so the primary utility row stays readable. */}
      <div className="lg:hidden">
        <div className="flex min-h-[52px] items-center justify-between px-4 py-2.5">
        ...
        <div className="border-t border-slate-100 px-4 py-1.5">
          <ConnectivityStatus
            compact
            className="w-full justify-center border-0 bg-transparent px-0 py-1"
          />
        </div>
      </div>
